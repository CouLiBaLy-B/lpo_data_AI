...
class ExceptionHandler:
    ...
    def handle_exception(exception: Exception, context: str = None) -> Dict[str, Any]:
        ...
        if isinstance(exception, SecurityException):
            logger.critical(f"Security violation: {exception.message}", extra={"context": context})
        elif isinstance(exception, DatabaseException):
            logger.error(f"Database error: {exception.message}", extra={"context": context})
        elif isinstance(exception, AIModelException):
            logger.warning(f"AI model error: {exception.message}", extra={"context": context})
        else:
            logger.error(f"Unhandled exception: {exception}", extra={"context": context})
