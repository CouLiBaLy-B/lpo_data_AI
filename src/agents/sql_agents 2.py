    ...
    def generate_sql_query(self, request: QueryRequest) -> SQLQuery:
        ...
        try:
            ...
            prompt_template = PromptTemplate(
                ...
                3. Respectez exactement les noms de colonnes de la base de données
                
                RETOUR:
                - Une requête SQL SELECT valide
                - Une explication de la requête
                - La confiance dans la requête
                - Le type de requête
                - Le nombre de lignes estimées
                
                EXEMPLE DE RETOUR:
                - Requête SQL: SELECT * FROM sensitive_areas
                - Explication: Utilise la table 'sensitive_areas' pour répondre à la question
                - Confiance: 0.9
                - Type de requête: SELECT
                - Nombre de lignes estimées: 1000
                """,
            )

            chain = LLMChain(llm=self.llm, prompt=prompt_template)
            response = chain.run({"question": request.question, "context": context})

            return SQLQuery(**response)

        except Exception as e:
            logger.error(f"Erreur lors de la génération de la requête SQL: {e}")
            raise AIModelException(f"Impossible de générer la requête SQL: {e}")
